...
FORCE_INLINE uint32_t remote_cb_local_pages_acked(uint32_t cb_id) {
    RemoteReceiverCBInterface& remote_cb = get_remote_receiver_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_acked_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_acked_ptr);
    return *pages_acked_ptr;
}

FORCE_INLINE void remote_cb_wait_front_fixed_pages_tracked(uint32_t cb_id, uint32_t num_pages, uint32_t pages_acked) {
    WAYPOINT("RCWF");
    RemoteReceiverCBInterface& remote_cb = get_remote_receiver_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_sent_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_acked_ptr - L1_ALIGNMENT);
    do {
        invalidate_l1_cache();
    } while (*pages_sent_ptr - pages_acked < num_pages);
    WAYPOINT("RCWD");
}

template <bool completion_fence = false>
FORCE_INLINE uint32_t
remote_cb_pop_front_fixed_pages(uint32_t cb_id, uint32_t num_pages, uint32_t pages_acked, uint8_t noc = noc_index) {
    RemoteReceiverCBInterface& remote_cb = get_remote_receiver_cb_interface(cb_id);
    uint32_t len_bytes = num_pages * remote_cb.fifo_page_size;
    uint32_t fifo_rd_ptr = remote_cb.fifo_rd_ptr;
    if (fifo_rd_ptr + len_bytes >= remote_cb.fifo_limit_page_aligned) {
        remote_cb.fifo_rd_ptr = remote_cb.fifo_start_addr + fifo_rd_ptr + len_bytes - remote_cb.fifo_limit_page_aligned;
    } else {
        remote_cb.fifo_rd_ptr += len_bytes;
    }
    // Fixed-page streams use monotonic absolute credits.  Each receiver has a
    // single producer, so an inline store avoids the atomic-return traffic and
    // finite atomic request accounting incurred by the generic byte-credit
    // protocol.
    uint32_t next_pages_acked = pages_acked + num_pages;
    volatile tt_l1_ptr uint32_t* pages_acked_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_acked_ptr);
    *pages_acked_ptr = next_pages_acked;
    uint64_t remote_ack_ptr_addr =
        get_noc_addr(remote_cb.sender_noc_x, remote_cb.sender_noc_y, (uintptr_t)pages_acked_ptr, noc);
    noc_inline_dw_write<InlineWriteDst::L1, !completion_fence>(remote_ack_ptr_addr, next_pages_acked, 0xF, noc);
    return num_pages;
}

FORCE_INLINE void remote_cb_wait_front_tracked(uint32_t cb_id, uint32_t num_pages, uint32_t pages_acked) {
    RemoteReceiverCBInterface& remote_cb = get_remote_receiver_cb_interface(cb_id);
    uint32_t len_bytes = num_pages * remote_cb.fifo_page_size;
    uint32_t fifo_limit_page_aligned = remote_cb.fifo_limit_page_aligned;
    if (remote_cb.fifo_rd_ptr + len_bytes >= fifo_limit_page_aligned) {
    ...
    do {
        invalidate_l1_cache();
    } while (*pages_sent_ptr - pages_acked < num_pages_wait);
}

FORCE_INLINE uint32_t remote_cb_pop_front(uint32_t cb_id, uint32_t num_pages, uint8_t noc = noc_index) {
    RemoteReceiverCBInterface& remote_cb = get_remote_receiver_cb_interface(cb_id);
    uint32_t len_bytes = num_pages * remote_cb.fifo_page_size;
    uint32_t fifo_limit_page_aligned = remote_cb.fifo_limit_page_aligned;
    uint32_t fifo_rd_ptr = remote_cb.fifo_rd_ptr;
...
FORCE_INLINE uint32_t remote_cb_local_pages_sent(uint32_t cb_id) {
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_sent_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_sent_ptr);
    return *pages_sent_ptr;
}

FORCE_INLINE void remote_cb_sender_barrier_tracked(uint32_t cb_id, uint32_t pages_sent) {
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_acked_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_sent_ptr + L1_ALIGNMENT);
    for (uint32_t i = 0; i < remote_cb.num_receivers; ++i) {
        do {
            invalidate_l1_cache();
        } while (*pages_acked_ptr != pages_sent);
        pages_acked_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
    }
}

FORCE_INLINE void remote_cb_reserve_back_tracked(uint32_t cb_id, uint32_t num_pages, uint32_t pages_sent) {
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    uint32_t len_bytes = num_pages * remote_cb.fifo_page_size;
    uint32_t fifo_limit_page_aligned = remote_cb.fifo_limit_page_aligned;
    uint32_t fifo_start_addr = remote_cb.fifo_start_addr;
    uint32_t fifo_size = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.config_ptr)[3];
    if (remote_cb.fifo_wr_ptr + len_bytes >= fifo_limit_page_aligned) {
        len_bytes += fifo_start_addr + fifo_size - fifo_limit_page_aligned;
    }
    uint32_t num_pages_wait = len_bytes / REMOTE_CIRCULAR_BUFFER_ALIGNED_PAGE_SIZE;
    uint32_t fifo_aligned_num_pages = fifo_size / REMOTE_CIRCULAR_BUFFER_ALIGNED_PAGE_SIZE;
    ...
    for (uint32_t i = 0; i < remote_cb.num_receivers; ++i) {
        ...
        do {
            invalidate_l1_cache();
            free_pages = fifo_aligned_num_pages - (pages_sent - *pages_acked_ptr);
        } while (free_pages < num_pages_wait);
        pages_acked_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
    }
}

FORCE_INLINE void remote_cb_reserve_back_fixed_pages_tracked(uint32_t cb_id, uint32_t num_pages, uint32_t pages_sent) {
    WAYPOINT("RCRB");
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    uint32_t fifo_size = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.config_ptr)[3];
    uint32_t fifo_num_pages = fifo_size / remote_cb.fifo_page_size;
    volatile tt_l1_ptr uint32_t* pages_acked_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_sent_ptr + L1_ALIGNMENT);
    for (uint32_t i = 0; i < remote_cb.num_receivers; ++i) {
        do {
            invalidate_l1_cache();
        } while (fifo_num_pages - (pages_sent - *pages_acked_ptr) < num_pages);
        pages_acked_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
    }
    WAYPOINT("RCRD");
}
...
FORCE_INLINE void remote_cb_sender_barrier(uint32_t cb_id) {
    ...
    for (uint32_t i = 0; i < num_receivers; ++i) {
        uint32_t pages_sent = *pages_sent_ptr;
        while (true) {
            // Snapshot the locally-owned sent count before invalidating the
            // cache line shared with the remotely-updated acknowledgement.
            invalidate_l1_cache();
            if (*pages_acked_ptr == pages_sent) {
                break;
            }
        }
        pages_acked_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
...
template <bool skip_ptr_update = true, bool completion_fence = false>
FORCE_INLINE uint32_t remote_cb_write_pages(
    uint32_t cb_id,
    ...
    for (uint32_t i = 0; i < num_receivers; ++i) {
        ...
        uint32_t last_src_addr = src_addr;
        uint32_t last_dest_addr = dest_addr;
        ...
        if constexpr (completion_fence) {
            // All bulk packets for this destination are posted.  A tiny
            // non-posted duplicate of the final word supplies one completion
            // acknowledgement per receiver without acknowledging every
            // payload packet.
            uint64_t fence_noc_addr = get_noc_addr_helper(remote_noc_xy, last_dest_addr);
            noc_async_write_one_packet<false, false>(last_src_addr, fence_noc_addr, sizeof(uint32_t), noc);
        }
        next_receiver_start_addr_offset += next_receiver_start_addr_stride;
        remote_noc_xy_ptr += 2;
    }

    uint32_t next_fifo_wr_ptr = fifo_wr_ptr + num_pages * remote_cb.fifo_page_size;
    if (next_fifo_wr_ptr >= fifo_limit_page_aligned) {
        next_fifo_wr_ptr = fifo_start_addr + (next_fifo_wr_ptr - fifo_limit_page_aligned);
    }
    remote_cb.fifo_wr_ptr = next_fifo_wr_ptr;
    return pages_sent;
}

template <bool posted_semaphore>
FORCE_INLINE void remote_cb_publish_pages(uint32_t cb_id, uint32_t aligned_pages_sent, uint8_t noc = noc_index) {
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_sent_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_sent_ptr);
    volatile tt_l1_ptr uint32_t* remote_noc_xy_ptr =
    ...
    for (uint32_t i = 0; i < remote_cb.num_receivers; ++i) {
        uint32_t remote_noc_xy = uint32_t(
            NOC_XY_ENCODING(DYNAMIC_NOC_X(noc, remote_noc_xy_ptr[0]), DYNAMIC_NOC_Y(noc, remote_noc_xy_ptr[1])));
        *pages_sent_ptr += aligned_pages_sent;
        uint64_t remote_sent_ptr_addr = get_noc_addr_helper(remote_noc_xy, (uint32_t)pages_sent_ptr);
        noc_semaphore_inc<posted_semaphore>(remote_sent_ptr_addr, aligned_pages_sent, noc);
        pages_sent_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
        remote_noc_xy_ptr += 2;
    }
}
...
template <bool completion_fence = false>
FORCE_INLINE void remote_cb_publish_fixed_pages(uint32_t cb_id, uint32_t pages_sent, uint8_t noc = noc_index) {
    RemoteSenderCBInterface& remote_cb = get_remote_sender_cb_interface(cb_id);
    volatile tt_l1_ptr uint32_t* pages_sent_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(remote_cb.aligned_pages_sent_ptr);
    volatile tt_l1_ptr uint32_t* remote_noc_xy_ptr =
    ...
    for (uint32_t i = 0; i < remote_cb.num_receivers; ++i) {
        ...
        *pages_sent_ptr = pages_sent;
        noc_inline_dw_write<InlineWriteDst::L1, !completion_fence>(
            get_noc_addr_helper(remote_noc_xy, (uint32_t)pages_sent_ptr), pages_sent, 0xF, noc);
        pages_sent_ptr += 2 * L1_ALIGNMENT / sizeof(uint32_t);
        remote_noc_xy_ptr += 2;
    }
}

template <bool skip_ptr_update = true, bool posted_semaphore = skip_ptr_update>
FORCE_INLINE void remote_cb_push_back_and_write_pages(
    ...
    uint32_t coalesced_page_size,
    uint8_t noc = noc_index) {
    uint32_t aligned_pages_sent = remote_cb_write_pages<skip_ptr_update>(
        cb_id, local_cb_addr, num_pages, num_rows, coalesced_num_pages_per_row, coalesced_page_size, noc);
    if constexpr (skip_ptr_update) {
        noc_async_posted_writes_flushed(noc);
    }
    remote_cb_publish_pages<posted_semaphore>(cb_id, aligned_pages_sent, noc);
}
#endif
#endif
