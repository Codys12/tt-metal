...
struct TunnelDescriptor {
    tt::ChipId mmio_id{std::numeric_limits<tt::ChipId>::max()};
    CoreCoord mmio_core_virtual{std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max()};
    CoreCoord mmio_core_logical{std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max()};

    tt::ChipId connected_id{std::numeric_limits<tt::ChipId>::max()};
    CoreCoord connected_core_virtual{std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max()};
    CoreCoord connected_core_logical{std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max()};

    int num_hops{0};

    tt_cxy_pair mmio_cxy_virtual() const { return tt_cxy_pair(static_cast<std::size_t>(mmio_id), mmio_core_virtual); }

    tt_cxy_pair connected_cxy_virtual() const {
        return tt_cxy_pair(static_cast<std::size_t>(connected_id), connected_core_virtual);
    }
};
