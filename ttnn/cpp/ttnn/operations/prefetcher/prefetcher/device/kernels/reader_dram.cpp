...
void kernel_main() {
    ...
    const uint32_t receiver_start = get_arg_val<uint32_t>(rt_args_idx++);
    const uint32_t receivers_per_bank = get_arg_val<uint32_t>(rt_args_idx++);
    const uint32_t* page_sizes = (uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_tensors)));
    const uint32_t* pages_per_row = (uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_tensors)));
    const uint32_t* block_num_tiles = (uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_tensors)));
    const uint32_t* full_row_bytes = (uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_tensors)));
    const uint32_t* block_heights = (uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_tensors)));
    ...
    {
        DeviceZoneScopedN("PREFETCH-DRAM-READ");
        for (uint32_t layer = 0; layer < num_layers; layer++) {
            for (uint32_t t = 0; t < num_tensors; t++) {
                uint32_t curr_page_size = page_sizes[t];
                uint32_t curr_pages_per_row = pages_per_row[t];
                uint32_t curr_block_num_tiles = block_num_tiles[t];
                uint32_t curr_full_row_bytes = full_row_bytes[t];
                uint32_t curr_block_height = block_heights[t];
                ...
                for (uint32_t block = 0; block < num_blocks; block++) {
                    ...
                    uint32_t src_row_addr = block * curr_block_height * curr_full_row_bytes +
                                            receiver_start * (curr_full_row_bytes / receivers_per_bank);
                    for (uint32_t h = 0; h < curr_block_height; ++h) {
                        uint32_t src_read_addr = src_row_addr;
                        for (uint32_t w = 0; w < curr_pages_per_row; ++w) {
                        ...
                        src_row_addr += curr_full_row_bytes;
                    }

                    if (num_free_blocks_in_buffer ==
                        3) {  // After first block we don't block but continue issuing reads
