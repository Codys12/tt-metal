...
def _fill_decode_kv_cache(mesh_device, kv_cache, attn_cfg, k_data, v_data):
    """Fill a decode KV cache with the same per-device KV assignment used by attention weights."""
    tp = mesh_device.shape[1] if hasattr(mesh_device, "shape") else 1
    if tp > 1:
        num_q_heads = attn_cfg.num_attention_heads
        q_per_device = num_q_heads // tp
        kv_replicated = attn_cfg.num_key_value_heads < tp
        local_kv = 1 if kv_replicated else attn_cfg.num_key_value_heads // tp
        for dev_idx in range(tp):
            if kv_replicated:
                kv_idx = (dev_idx * q_per_device) * attn_cfg.num_key_value_heads // num_q_heads
...
def test_layer_forward_decode_with_gemma4_rope_cache(layer_idx, mesh_device):
    """
    Full decoder-layer decode PCC using Gemma4Model's local RoPE cache builder.

    This exercises the production decode path where RoPE caches are 2D and the
    current position is gathered on device with ttnn.embedding.
    """
    from transformers.cache_utils import DynamicCache
    from transformers.models.gemma4.modeling_gemma4 import Gemma4TextRotaryEmbedding
    ...
    with torch.no_grad():
        hf_output = hf_layer(
            x_torch,
            per_layer_input=_make_per_layer_input(hf_text_config, 1),
            position_embeddings=(cos, sin),
            past_key_values=hf_cache,
            attention_mask=mask,
        )
    ...
    passing, pcc_msg = compare_tensors(tt_output_torch, hf_output, pcc_threshold=0.90)
    assert passing, (
        f"Full layer decode with Gemma4 RoPE cache (layer_idx={layer_idx}, layer_type={layer_type}, tp={tp}) "
        f"PCC too low: {pcc_msg}"
    )
