...
def is_moe_model():
    ...
    try:
        ...
        # Model type unknown to the installed transformers (e.g. diffusion_gemma):
        # read config.json directly. num_experts implies the MoE block.
        import json

        try:
            with open(os.path.join(_get_model_path(), "config.json")) as f:
                tc = json.load(f).get("text_config", {})
            return bool(tc.get("enable_moe_block", tc.get("num_experts")))
        except OSError:
            return False
...
def parametrize_mesh_with_fabric(mesh_shapes=None):
    ...
    Default shapes: (1,1) single card, (1,2) N300, (1,4) half-T3K / N150x4, (1,8) T3K.

    The (1,4) variant is what each replica sees when a 1x8 parent is split into
    two data-parallel halves (see ``parametrize_mesh_for_dual_replica`` for the
    paired-submesh case).
    ...
    Usage:
        ...
        pytest -k "1x4"   # half-T3K / N150x4 (TP=4)
        pytest -k "1x8"   # T3K (TP=8)
    """
    num_devices = ttnn.get_num_devices()

    if mesh_shapes is None:
        all_shapes = [(1, 1), (1, 2), (1, 4), (1, 8)]
