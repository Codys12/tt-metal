...
class Gemma4ModelArgs:
    ...
    def load_state_dict(weights_path, dummy_weights=False):
        """Load model state dict from safetensors (lazy, mmap-backed) or HF checkpoint."""
        if dummy_weights:
            return {}
        ...
        if safetensor_files:
            # Lazy view: tensors are read (and fp32->bf16 converted) on access, so
            # peak RAM tracks what the caller actually holds rather than the full
            # checkpoint. Needed on hosts that cannot mmap a 47 GiB shard.
            from models.demos.gemma4_cody.utils.lazy_state_dict import LazyStateDict

            logger.info(f"Opening lazy state dict over {len(safetensor_files)} safetensor files from {weights_path}")
