...
import ttnn
from models.demos.gemma4_cody.config import MeshConfig
...
def load_attention_weights(
    ...
    wqkv_cache_name = get_cache_file_name(tensor_cache_path, f"wqkv{tp_suffix}")
    o_proj_cache_name = get_cache_file_name(tensor_cache_path, f"o_proj{o_proj_cache_suffix}{tp_suffix}")
    q_norm_cache_name = get_cache_file_name(tensor_cache_path, f"q_norm.weight{tp_suffix}")
    k_norm_cache_name = get_cache_file_name(tensor_cache_path, f"k_norm.weight{tp_suffix}")

    qkv = cached_tensor_placeholder(wqkv_cache_name, weight_dtype, ttnn.TILE_LAYOUT)
    o_w = cached_tensor_placeholder(o_proj_cache_name, weight_dtype, ttnn.TILE_LAYOUT)
    q_norm_w = cached_tensor_placeholder(q_norm_cache_name, ttnn.bfloat16, ttnn.ROW_MAJOR_LAYOUT)
    k_norm_w = cached_tensor_placeholder(k_norm_cache_name, ttnn.bfloat16, ttnn.ROW_MAJOR_LAYOUT)
    ...
    wqkv = ttnn.as_tensor(
        ...
        cache_file_name=wqkv_cache_name,
    ...
    o_proj = ttnn.as_tensor(
        ...
        cache_file_name=o_proj_cache_name,
        memory_config=ttnn.DRAM_MEMORY_CONFIG,
    )
    q_norm_weight = ttnn.as_tensor(
        ...
        cache_file_name=q_norm_cache_name,
        memory_config=ttnn.DRAM_MEMORY_CONFIG,
    )
    k_norm_weight = ttnn.as_tensor(
        ...
        layout=ttnn.ROW_MAJOR_LAYOUT,
        mesh_mapper=replicate_mapper,
        cache_file_name=k_norm_cache_name,
        memory_config=ttnn.DRAM_MEMORY_CONFIG,
    )
