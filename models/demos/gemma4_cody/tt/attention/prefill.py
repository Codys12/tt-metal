...
def prefill_forward(
    ...
    # Sliding layers use a separate page table that routes logical blocks to a
    # ring buffer (with out-of-window blocks pointed at a scratch row).
    if kv_cache is not None and shared_kv is None:
        k_cache, v_cache = kv_cache
        # (paged_)fill_cache sizes its CB from input.dtype but writes raw tiles
        # into the cache buffer. With a block-float cache and bf16 input, the
        # tile sizes mismatch and the cache gets garbage. Match dtypes here.
        # paged_update_cache (decode) handles this internally so no equivalent
        # cast is needed there.
        if tt_k.dtype != k_cache.dtype:
            tt_k_pre, tt_v_pre = tt_k, tt_v
            tt_k = ttnn.typecast(tt_k, k_cache.dtype)
            tt_v = ttnn.typecast(tt_v, v_cache.dtype)
            tt_k_pre.deallocate(True)
            tt_v_pre.deallocate(True)
        ...
        if layer_page_table is not None:
            ttnn.experimental.paged_fill_cache(k_cache, tt_k, layer_page_table, batch_idx=user_id)
            ttnn.experimental.paged_fill_cache(v_cache, tt_v, layer_page_table, batch_idx=user_id)
        else:
